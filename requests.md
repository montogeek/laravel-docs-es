...
To obtain an instance of the current HTTP request via dependency injection, you should type-hint the class on your controller constructor or method. The current request instance will automatically be injected by the [service container](/5.0/container):
...
The `flash` method will flash the current input to the [session](/5.0/session) so that it is available during the user's next request to the application:
