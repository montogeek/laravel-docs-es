...
If you need control over the response status and headers, but also need to return a [view](/{{version}}/views) as the response content, you may use the `view` method:
...
Of course, after the user is redirected to a new page, you may retrieve and display the flashed message from the [session](/{{version}}/session). For example, using [Blade syntax](/{{version}}/blade):
...
For example, from a [service provider's](/{{version}}/providers) `boot` method:
